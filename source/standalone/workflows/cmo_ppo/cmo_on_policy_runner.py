...
from cmo_ppo import CMOPPO


class CMOOnPolicyRunner():
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        cost_critic = ActorCritic(
            ...
            num_actions=1,
            ** self.policy_cfg.get("cost_critic", {})
        )

        for key in ["class_name", "schedule", "desired_kl", "symmetry_cfg", "rnd_cfg", "cost_coef_init"]:
            self.alg_cfg.pop(key, None)
        ...
        # Disable logging on non-root processes
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                ...
                if self.training_type == "rl":
                    self.alg.compute_returns_and_costs(privileged_obs)
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            # everything else
            if "mean_cost" in locs["loss_dict"]:
                self.writer.add_scalar("Train/mean_cost", locs["loss_dict"]["mean_cost"], locs["it"])
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            if "mean_cost" in locs["loss_dict"]:
                log_string += f"""{'Mean cost:':>{pad}} {locs['loss_dict']['mean_cost']:.2f}\n"""
            log_string += f"""{'Mean cost:':>{pad}} {statistics.mean(locs['costbuffer']):.2f}\n"""
            # -- episode info
    ...
    def save(self, path: str, infos=None):
        ...
        if hasattr(self.alg, "lagrangian_optimizer"):
            saved_dict["lagrangian_optimizer_state_dict"] = self.alg.lagrangian_optimizer.state_dict()
        if hasattr(self.alg, "lagrange_multiplier"):
            saved_dict["lagrange_multiplier"] = self.alg.lagrange_multiplier.item()
